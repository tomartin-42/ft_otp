...
std::string AES_g::encrypt(const std::string &plaintext, const std::string &key,
                           const std::string &iv) {
  ...
  if (EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, key_data, iv_data) !=
      1) {
  ...
  if (EVP_EncryptUpdate(
          ctx, reinterpret_cast<unsigned char *>(&ciphertext[0]),
          &ciphertext_len,
  ...
  if (EVP_EncryptFinal_ex(
          ctx, reinterpret_cast<unsigned char *>(&ciphertext[ciphertext_len]),
          &final_len) != 1) {
    EVP_CIPHER_CTX_free(ctx);
    throw std::runtime_error("Error de cifrado.");
  }
...
std::string AES_g::decrypt(const std::string &ciphertext,
  ...
  if (EVP_DecryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, key_data, iv_data) !=
      1) {
    EVP_CIPHER_CTX_free(ctx);
    throw std::runtime_error(
        "No se pudo inicializar el contexto de descifrado.");
  }
  ...
  if (EVP_DecryptFinal_ex(
          ctx, reinterpret_cast<unsigned char *>(&plaintext[plaintext_len]),
          &final_len) != 1) {
    EVP_CIPHER_CTX_free(ctx);
